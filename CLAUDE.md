# CLAUDE.md — Camp DaddyMan Project Memory

> Keep this file LEAN. It loads into context every session.
> Point to documents; do not paste their full text here. Never paste long transcripts
> into the session — that pushes context past 200K and triggers paid 1M-context billing.
...
- **DaddyMan Studios** — creative house (film, music, literature, transmedia)
- **DaddyMan Publishing** — publishing arm (formerly SunMan Publishing — NO LONGER USED)
- **Camp DaddyMan** — youth development arm
- Nonprofit umbrella: **D.A.D.D.Y. Inc.** (501c3)
- Keep the studio (for-profit) and the nonprofit structurally SEPARATE. Never conflate.

## Governing documents (read on demand — do NOT inline)
- `The_Ark_Constitution` — the locked philosophy and full framework
- `THE_FOUR_TESTS.md` — the build/decision filter (see below)
- `The_Service_to_Rank_Rule_Set` — how rank is earned by service, never money
- `First_Acts_Loop_One_Playbook` / charter — the real-world pilot

## Transformation framework (LOCKED — 8 stations)
The lifecycle is a CIRCLE, not a ladder. 5 stations are climbable; 3 are conferred, never climbed.
...
2. **Egg Test** — does it help them pollinate / lay eggs? (purpose — better)
3. **Field Test** — does it improve the FIELD, not just the individual?
4. **Father Test** — does it make them MORE CAPABLE of standing without us,
   or just more dependent? (the audit)
- Beneath all: **does this create more life than it consumes?**
...
- Capability over retention. Choose maturity even when it costs retention.
- Minors: all mentoring runs through D.A.D.D.Y. Inc. safeguarding (background checks,
  second adult present, no unsupervised one-on-one).
- **Drew's Patois is never sanitized** — every spelling and construction is deliberate.
- All formal documents: built with Node.js + the `docx` library; **no background shading.**

## Brand
- daddymanstudios.com = DaddyMan Studios (parent creative house)
- campdaddyman.com = Camp DaddyMan (youth arm). Never conflate the two.

## Working notes
- Web stack history: Joomla (T4 + SP Page Builder) for some sites; Node.js/Express/MongoDB
  + AWS S3/CloudFront backend for the Camp platform. Palette: garrison green #00C832, gold #FFD700.
- When a session gets heavy, summarize decisions INTO files and clear context —
  don't let the transcript live in the window.
